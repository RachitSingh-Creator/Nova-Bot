...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    try {
      await signup({ email, full_name: fullName || null, password });
      navigate("/login");
    } catch (err) {
  ...
  return (
    <div className="min-h-screen grid place-items-center bg-gradient-to-br from-slate-100 to-cyan-100 dark:from-slate-950 dark:to-slate-900 p-4 text-slate-900 dark:text-slate-100">
      <form onSubmit={handleSubmit} className="w-full max-w-sm rounded-2xl bg-white/90 dark:bg-slate-900/90 p-6 shadow-xl border border-slate-200/70 dark:border-slate-700">
        ...
        <p className="text-base text-slate-700 dark:text-slate-300 mb-4">Create account</p>
        <input value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Full name" className="mb-3 w-full rounded-lg border border-slate-300 dark:border-slate-600 px-3 py-2 bg-white/80 dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder:text-slate-500 dark:placeholder:text-slate-400" />
        ...
        <button className="w-full rounded-lg bg-sky-600 hover:bg-sky-500 text-white py-2">Signup</button>
        <button type="button" onClick={() => navigate("/login")} className="w-full mt-2 text-sm text-sky-700 dark:text-sky-300 hover:underline">Back to login</button>
      </form>
    </div>
  );
}
